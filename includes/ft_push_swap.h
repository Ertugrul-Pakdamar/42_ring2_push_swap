/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_push_swap.h                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: epakdama <epakdama@student.42istanbul.c    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/21 14:31:40 by epakdama          #+#    #+#             */
/*   Updated: 2025/07/24 09:08:18 by epakdama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef FT_PUSH_SWAP_H
# define FT_PUSH_SWAP_H

# define SA 1
# define SB 2
# define SS 3
# define PA 4
# define PB 5
# define RA 6
# define RB 7
# define RR 8
# define RRA 9
# define RRB 10
# define RRR 11

# include "libft.h"
# include <limits.h>
# include <stdlib.h>
# include <unistd.h>

typedef struct s_stack
{
	int				content;
	long			id;
	struct s_list	*next;
}					t_stack;

t_stack				*ft_set_list(char **argv);
void				ft_exit_prog(t_stack **stack_a, t_stack **stack_b,
						void ***arr, int exit_code);
int					ft_is_input_valid(char *arg);
void				ft_rule_base(t_stack **stack_a, t_stack **stack_b,
						int rule);
void				ft_swap_stack(t_stack **stack);
void				ft_push_stack(t_stack **stack_a, t_stack **stack_b);
void				ft_rotate_stack(t_stack **stack);
void				ft_reverse_rotate_stack(t_stack **stack);
int					ft_custom_atoi(char *str, int *res);

#endif